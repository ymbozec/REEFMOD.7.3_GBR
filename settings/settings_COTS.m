...
META.COTS_mortality(5:end) = META.COTS_mortality(5:end)/6; % AFTER CALIBRATION WITH LIZARD IS (used until SEP 2025)
% META.COTS_mortality = 1 - ((1 - META.COTS_mortality).^0.680); % SEP 2025: NEW CALIBRATION FROM SUKI
% Obtained after optimisation of the exponent of mortality adjustment + parameters of the larval-stock recruitment for CoTS
% against manta tow observations. simulations using new CoTS connectivity (GBRLUP) from CCIP.

%Tina: For altering mortality for outside green zones: 1 - (1 - m)^a  %m = mortality-at-age in lit, a = coefficient to change m
META.COTS_mortality_spec = zeros(16,1);
META.COTS_mortality_spec(1) = META.COTS_mortality(1); %Keep settlers same
META.COTS_mortality_spec(2:end) = 1 - ((1 - META.COTS_mortality(2:end)) .^0.7632); %Alter all >6mo
...
META.COTS_background_density_INIT_BOX = 0.1; % Background density in the CoTS initiation box
...
% PARAMETERISATION of the B-H function as in Bozec et al. 2022
...
% NEW PARAMETERISATION FROM SUKI (optimised alobgside mortality against manta tow obs)
% META.COTS_BH_alpha = 385236 ; % Suki 29/09/2025: fitted to GBRLUP cycle
% % which is ~960 settlers per m2 - (400m2 grid)
% META.COTS_BH_beta = 1281820 ; % Suki 29/09/2025: fitted to GBRLUP cycle
...
% META.COTS_min_selfseed = 0 ; % Suki 29/09/2025: fitted to GBRLUP cycle
