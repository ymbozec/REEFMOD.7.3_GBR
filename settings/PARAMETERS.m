...
% Now mortality of large adult is size specific, declining from M0 = adol_whole_mortality_rate as:
% M0*exp(-3*coral_cm2./adult_max_size); (further multiplied sensitivity coefficient
...
REEF.initial_algal_cover = [ 0 ; 0.05 ; 0.05 ; 0 ]; % Turf/EAM are estimated during initalisation automatically
...
%__________________________________________________________________________________________
%
%       SET UP CYCLONES/HURRICANES
%__________________________________________________________________________________________

META.doing_hurricanes = 0 ; % set to 0/1 to switch off/on hurricanes
META.deterministic_hurricane_mortality = 0; % option for generating deterministic (1) or stochastic (0) mortalities from a given cyclone category

META.randomize_hurricane_chronology = 0 ; % set to 1 to randomize hurricane chronology for each simulation

META.hurricane_strike_incidence = 1; % Probability that a predicted storm actually strikes (binomial draw)
...
C_adjust_GBR = 5 ; % calibration of cyclone mortality with AIMS LTMP transect data (Bozec et al. 2022)
...
META.deterministic_bleaching = 0; % option for generating deterministic (1) vs stochastic (0) mortalities from DHWs
...
% Coeff for adjusting Hughes' initial bleaching mortality to long-term (6mo) mortality
% (based on calibration with post-bleaching cover loss, Bozec et al. 2022)
