...
if t<=META.nb_time_steps %otherwise stops controlling one step before the last step
    ...
    COTS_records=struct('COTS_criteria',[],'control_record',[],'COTS_initial',[],'COTS_final',[]);
    ...
    %current_COTS_ET=repmat(0.075,[3806 1]);
     current_COTS_ET=repmat(0.075,[size(idx) 1]); % YM & Tina (Mar 2024): fixed ET as in Castro et al (2023). Deliberate choices based on a few reasons, 
     % including that GBRMPA did not always implement a dynamic ET during the control program
%    current_COTS_ET = f_calculate_COTS_ET(RESULT,t, META); % YM (Mar 2024): ET function of current cover of preferred coral preys
    
    %COTS_records.COTS_ET=current_COTS_ET; Tina commented
    ...
    [~, criteriaS, global_trigger, RESULT] = f_makeReefList_TS(META, RESULT, t, 0, current_COTS, thisboatorder, current_COTS_ET, COTS_site_densities);
    
%     if isreal(current_COTS)==0  %Could be 0/1, check for square roots etc too.
%         current_COTS
%     end   
    ...
    control_records=struct('reef',[],'sites_dives',[],'sites_over_ET',[]);
    cnt1=1;
            ...
            while remaining_dives>0 && current_reef<size(criteria,1) % YM April 10 2022 - add constraint for nb of simulated reefs
                ...
                if isempty(newid)==0 %% YM: corrected for sizeable code
                    ...
                    if ~isempty(sites_over_ET) && remaining_dives>0%if there are sites to treat, and dives remaining, do control
                        ...
                        %control_records(cnt1).this_sites_over_ET=this_sites_over_ET;Tina: don't need logical as well
                        ...
                        %we need to rescale the COTS post-control proportions, or cpcp, to the fact that only
                        ...
                        while site<=length(sites_over_ET) && remaining_dives>0%go throguh all sites
                            ...
                            control_dives=(ceil((4.18)*(this_site_tow/0.015)^0.667)); %indiv dives
                            %control_dives=(ceil((4.18/META.boatProperties.divers(1))*(this_site_tow/0.015)^0.667)); team dives
                            remaining_dives_tmp = remaining_dives - control_dives; 
                            %remaining_dives=remaining_dives-control_dives;%subtract perfomed dives from total
                            if remaining_dives_tmp < 0 %break if no more dives left
                               control_dives = remaining_dives_tmp + control_dives;
                               remaining_dives = 0;
                            else remaining_dives = remaining_dives_tmp;
                            end
                    ...
                    %control_records(cnt1).COTS_post_control_overall_ET=site_ETtow;%record overall ET for this reef/site; note that ET is decided at reef level the same as coral cover and is the same for all sites. Tina: don't need
                    cnt1=cnt1+1;
...
%     RESULT.COTS_culled_reefs(1,t) = culled_reefs; %(YM: in fact they were the visited reefs, not necessarily culled)
    ...
    RESULT.COTS_control_remaining_dives(1,t) = remaining_dives; % 03/2025: remaining_dives is always 0!
