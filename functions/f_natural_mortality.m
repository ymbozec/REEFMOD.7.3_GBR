...
adult_max_size = id0 ; % March 2025: testing whole-colony mortality of adults function of size
...
for s = 1:nb_coral_types
    ...
    % March 2025: etxract max sizes for whole-colony mortality of adults function of size
    adult_max_size(:,col_start:col_stop) = CORAL.adult_max_size(s);

    col_start = col_start + species_ID(s) ;
end

% March 2025: testing whole-colony mortality of adults function of size
% First set mortality rate of sub-adults:
adol_whole_mortality_rate = REEF.adol_whole_mortality_rate*sensitivity_whole_natural;
% Then set mortality rate of adults, as declining function with size from subadult mortality rate
a = 3;
M0 = REEF.adol_whole_mortality_rate; % maximum mortality of adult = mortality of the largest subadult
adult_whole_mortality_rate = M0*sensitivity_whole_natural.*exp(-a*coral_cm2./adult_max_size);
...
id_adol(rand_mort_adol > adol_whole_mortality_rate) = 0 ;
id_adult(rand_mort_adult > adult_whole_mortality_rate) = 0 ;
