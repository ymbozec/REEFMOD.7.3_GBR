...
% Works both for coral outplanting and larval enrichment
% Revisited Mar 2024
% -------------------------------------------------------------------------
...
% Mar 2024: note 'outplants' is a generic term referring to either outplants from coral aquaculture or larvae seeded on the reef
% 'Outplanting' is processed at the scale of a grid cell
...
%     avail_space_cm2 = algal(1).cover_cm2 + algal(4).cover_cm2 ; % can be only outplanted on EAM and TURF
    avail_space_cm2 = algal(1).cover_cm2 ; % can be only outplanted on EAM
    ...
    if META.deploy_random_density == 1
...
%         nb_outplants(nb_outplants > 5) = 5;
    ...
    transplant_layer_cm2(restored_cells,:) = ceil(pi*(transplant_layer(restored_cells,:)/2).^2); % Note the rounding gives 0 cm2 for diameters <0.8 (extremely rare)
    ...
    coral(s).heat_tolerance = [coral(s).heat_tolerance HT_outplants];
...
algal(1).cover_cm2(algal(1).cover_cm2<0)=0; % final adjustment of EAM after deployment. In rare cases EAM might be negative so forced to 0.